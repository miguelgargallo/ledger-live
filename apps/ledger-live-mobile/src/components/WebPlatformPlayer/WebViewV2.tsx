...
function useWebViewHandlers({
  onMessageRaw,
  onReloadRaw,
  setLoadDate,
}: {
  onMessageRaw: ReturnType<typeof useWalletAPIServer>["onMessage"];
  onReloadRaw: ReturnType<typeof useWalletAPIServer>["onReload"];
  setLoadDate: (date: Date) => void;
}) {
...
function useWebView({ manifest, inputs }: Pick<Props, "manifest" | "inputs">) {
  ...
  const config = useConfig({
    ...
    wallet: { name: packageInfo.name, version: packageInfo.version },
  });
  ...
  const { onMessage, onReload } = useWebViewHandlers({
    onMessageRaw,
    onReloadRaw,
    setLoadDate,
  });
